package org.concurrent.chapter01.executor;
 ...
 * 1、CompletionService接口：目前JDK中只有一个实现类ExecutorCompletionService，该类使用创建时提供的Executor对象（通常是线程池）来执行任务。
 * 然后，将结果放入一个阻塞队列中。
 * submit() -- 提交任务；
 * take() -- （阻塞）获取任务结果，等待下一个结果并返回Future对象；
 * poll() -- （非阻塞）获取任务结果，不等待或者等待指定时间，有结果就返回一个Future对象，否则返回null。
 *
 * 2、demo中展示了ExecutorCompletionService的并发使用方式，并且对并发操作数据的情况进行了简单的测试。
 *
 * Created by nn_liu on 2017/4/13.
 */
public class ExecutorCompletionServiceDemo {
    ...
    /**
     * 并发的情况下操作sum数据会有问题(需要添加锁的限制)
     *
     * @throws InterruptedException
     * @throws ExecutionException
     */
    static void case4() throws InterruptedException, ExecutionException {
        ExecutorService service = Executors.newFixedThreadPool(10);
        ExecutorCompletionService<Integer> completionService = new ExecutorCompletionService<Integer>(service);

        final Sum sum = new Sum(0);
        final Random random = new Random();
        ...
        for (int i = 0; i < 100; i++) {
            completionService.submit(new Callable<Integer>() {
                ...
                public Integer call() throws Exception {
                    Integer result = sum.getSum();
                    result += 1;
                    sum.setSum(result.intValue());
                    return result;
                }
            });
        }
